...
unsigned int SPI_Ethernet_UserTCP(unsigned char *remoteHost, unsigned int remotePort, unsigned int localPort, unsigned int reqLength, TEthPktFlags *flags) {
    ...
    for (getRequestLen=0; getRequestLen<10; ++getRequestLen) {
        getRequest[getRequestLen]=SPI_Ethernet_getByte();
    }
    getRequest[getRequestLen]=0;
    ...
    length += SPI_Ethernet_putString(StrtPage);
    return length;
}
